function Start-JS7ExecutableFile
{
<#
.SYNOPSIS
...
.EXAMPLE
$process = Start-JS7ExecutableFile -Path 'c:/tmp/powershell/sample_script.cmd' -TargetName 'run_as_ap'
...
.EXAMPLE
$process = Start-JS7ExecutableFile -Path 'c:/tmp/powershell/sample_script.cmd' -TargetName 'run_as_ap' -NoStandardOutput -NoStandardError
...
#                   $process = Start-Process -FilePath 'cmd.exe' "/c ""`"$Path`" $Argumentlist"" " -NoNewWindow -PassThru -Wait -Credential $credentials -RedirectStandardOutput $tempStdoutFile -RedirectStandardError $tempStderrFile
                    ...
                    if ( $PSCmdlet.ShouldProcess( $Path, 'Start-Process' ) )
                    {
